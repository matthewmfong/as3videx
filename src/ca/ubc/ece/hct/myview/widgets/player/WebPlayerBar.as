...
import org.osflash.signals.Signal;
	...
	public class WebPlayerBar extends Sprite {

		private static const SEEKBAR_HEIGHT:Number = 4;
		private static const TOOLBAR_HEIGHT:Number = 27;
		...
		private var seekbarEnabled:Boolean = true;
		...
		public function WebPlayerBar(video:VideoMetadata, width:Number, totalTime:Number) {
			super();
			_video = video;
			...
			seekbar = new SeekBar(video, width, SEEKBAR_HEIGHT);
			seekbar.y = 0;
			seekbar.clicked.add(
				function(newTime:Number):void {
					seekSignal.dispatch(currentPlayTime, newTime);
				})
			// drawSeekbar();
			addChild(seekbar);
			seekbar.showPreviewThumbnail = false;
			...
			// var arial = new Arial();
			// var timestampFormat:TextFormat = new TextFormat("Arial", 20, 0xffffff);
			// timestampFormat.size = 15;
			// timestampFormat.font = arial.fontName;
			// timestampFormat.color = 0xffffff;

			timeLabel.defaultTextFormat = new TextFormat("Arial", 12, 0xffffff);;
		...
		public function set video(val:VideoMetadata):void {
			_video = val;
			// previewThumbnail.video = val.filename;
			seekbar.video = val;
		}

		public function set showEnlarge(val:Boolean):void {
			if(val && !toolbar.contains(enlargeButton)) {
				toolbar.addChild(enlargeButton);
			} else if(!val && toolbar.contains(enlargeButton)) {
				toolbar.removeChild(enlargeButton);
			}
		}

		public function set showCC(val:Boolean):void {
			if(val && !toolbar.contains(ccButton)) {
				toolbar.addChild(ccButton);
			} else if(!val && toolbar.contains(ccButton)) {
				toolbar.removeChild(ccButton);
			}
		}

		public function set showPlaybackRate(val:Boolean):void {
			if(val && !toolbar.contains(playbackRateLabel)) {
				toolbar.addChild(playbackRateLabel);
			} else if(!val && toolbar.contains(playbackRateLabel)) {
				toolbar.removeChild(playbackRateLabel);
			}
		}
		...
		public function drawHighlights():void {
			...
			// 	highlightSprite.graphics.drawRect(0, 0, _width, _video.highlightColours.length * lineThickness);
			// 	highlightSprite.graphics.endFill();

			// 	for(var i:int = 0; i<_video.highlightColours.length; i++) {

			// 		for(var j:int = 0; j<_video.highlights[i].length; j++) {

			// 			if(0 <= _video.highlights[i][j].end && _video.duration >= _video.highlights[i][j].start) {
							
			// 				var start:Number = (_video.highlights[i][j].start)/totalTime * _width;
			// 				var end:Number = (_video.highlights[i][j].end)/totalTime * _width;
			// 				if(start < 0)
			// 					start = 0;
			// 				if(end > _width)
			// 					end = _width;

			// 				highlightSprite.graphics.beginFill(Util.changeSaturation(_video.highlightColours[i], 3), 1);
			// 				highlightSprite.graphics.drawRect(start, (i)*lineThickness, (end-start), lineThickness);
			// 				highlightSprite.graphics.endFill();

			// 				highlightSprite.graphics.beginFill(Util.brighten(_video.highlightColours[i], 1.1), 1);
			// 				highlightSprite.graphics.drawRect(start, (i)*lineThickness, end-start, 1);
			// 				highlightSprite.graphics.endFill();
			// 				highlightSprite.graphics.beginFill(Util.brighten(_video.highlightColours[i], 0.9), 1);
